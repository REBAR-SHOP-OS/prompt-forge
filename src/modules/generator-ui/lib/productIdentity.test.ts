...
describe('editing a selected product description', () => {
  const product = normalizeProductIdentity({
    id: 'folder:stirrups', category: 'products', url: 'front.png', urls: ['front.png', 'side.png'], description: 'old',
  })
  const legacy = { ...product, id: 'legacy', description: 'legacy old' }
  const other = { ...product, id: 'unrelated' }

  it('maps an edited angle to grouped selections in every draft without touching other products', () => {
    const identities = { first: product, second: product, other }
    const updates = productDescriptionUpdates(groups, ['side'], 'new description')
    const next = applyProductDescriptionUpdates(identities, updates)
    expect(next.first.description).toBe('new description')
    expect(next.second.description).toBe('new description')
    expect(next.first.urls).toEqual(['front.png', 'side.png'])
    expect(next.other).toBe(other)
    expect(product.description).toBe('old')
  })

  it('supports bulk imports, legacy image IDs and clearing descriptions', () => {
    const updates = productDescriptionUpdates(groups, ['front', 'side', 'legacy'], null)
    const next = applyProductDescriptionUpdates({ grouped: product, legacy }, updates)
    expect(next.grouped.description).toBeNull()
    expect(next.legacy.description).toBeNull()
  })

  it('persists edits for unloaded drafts and leaves another user storage untouched', () => {
    const key = 'project-product-identities:user-a'
    const otherKey = 'project-product-identities:user-b'
    localStorage.setItem(key, JSON.stringify({ active: product, unloaded: product, other }))
    localStorage.setItem(otherKey, JSON.stringify({ draft: product }))
    const updates = productDescriptionUpdates(groups, ['front'], 'saved caption')
    persistProductDescriptionUpdates(localStorage, key, updates)
    const reloaded = JSON.parse(localStorage.getItem(key)!)
    expect(activeProjectProductIdentity(reloaded, 'unloaded')?.description).toBe('saved caption')
    expect(reloaded.other.description).toBe('old')
    expect(JSON.parse(localStorage.getItem(otherKey)!).draft.description).toBe('old')
    expect(dashboardSource).toContain('syncProductDescriptions(targetIds, text)')
    expect(dashboardSource).toContain('syncProductDescriptions([imageId], value)')
  })

  it('keeps edits when signing finishes late without resurrecting a cleared selection', async () => {
    let finish: ((url: string) => void) | undefined
    const restoring = refreshProductIdentity({ ...product, urls: [product.url] }, () => new Promise<string>((resolve) => { finish = resolve }))
    const updates = productDescriptionUpdates(groups, ['front'], 'edited while loading')
    finish!('fresh-front')
    const restored = await restoring
    const edited = applyProductDescriptionUpdates({ draft: restored! }, updates)
    expect(mergeRestoredProductIdentities(edited, {}, new Set()).draft.description).toBe('edited while loading')
    expect(edited.draft.url).toBe('fresh-front')
    expect(mergeRestoredProductIdentities(edited, {}, new Set(['draft']))).toEqual({})
  })
})
