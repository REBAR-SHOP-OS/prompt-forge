...
Each scene should flow logically into the next, building toward a single cohesive narrative. All scenes must serve the same overall story goal.`
  ...
  const selectedCameraLabel = CAMERA_ANGLES.find((a) => a.value === selectedCameraAngle)?.label ?? 'None'
  const selectedThemeLabel = THEMES.find((t) => t.value === selectedTheme)?.label ?? 'None'
  ...
  return (
    <>
      ...
      <StylePickerDialog
        ...
        title="Camera angle"
        description="Choose how the camera moves. The previous selection stays until you press Apply."
        value={selectedCameraAngle}
        noneValue="auto"
        noneLabel="None (AI decides)"
        options={CAMERA_PICKER_OPTIONS}
        onApply={setSelectedCameraAngle}
      />
      ...
      <StylePickerDialog
        ...
        title="Visual theme"
        description="Pick a genre, scene or video template. The previous selection stays until you press Apply."
        value={selectedTheme}
        noneValue="auto"
        noneLabel="None (AI decides)"
        options={THEMES.map(toPickerOption)}
        tabs={THEME_TABS}
