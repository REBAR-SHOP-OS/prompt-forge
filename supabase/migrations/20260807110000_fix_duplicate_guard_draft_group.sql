-- Fix: Allow multiple jobs with the same prompt when they share a draft_group_id
-- This fixes the DUPLICATE_JOB error when generating multi-scene videos (30s+)
-- where each scene legitimately needs its own job but shares the same prompt.
...
CREATE OR REPLACE FUNCTION public.generator_start_job_v2(
  ...
  _first_frame_url text DEFAULT NULL::text,
  _last_frame_url text DEFAULT NULL::text,
  _reference_image_urls text[] DEFAULT NULL::text[],
  _requested_aspect_ratio text DEFAULT NULL::text,
  _requested_duration integer DEFAULT NULL::integer,
  _draft_group_id uuid DEFAULT NULL::uuid,
  _narration_text text DEFAULT NULL::text
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Duplicate guard: prevent accidental double-submits of the same prompt.
  -- BUT: allow multiple jobs that share a draft_group_id (multi-scene films).
  SELECT EXISTS(
    ...
    WHERE user_id = _user_id
      ...
      AND NOT (_draft_group_id IS NOT NULL AND draft_group_id = _draft_group_id)
  ) INTO _duplicate;
  IF _duplicate THEN
