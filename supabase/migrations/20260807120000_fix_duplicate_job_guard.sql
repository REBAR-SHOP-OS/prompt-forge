-- Fix DUPLICATE_JOB false-positives for multi-scene submissions.
--
-- When a user generates a 30s/45s/135s video, the frontend splits the prompt
-- into multiple scenes and calls createJob() for each scene in quick succession.
-- The frontend gateway auto-generates a unique clientRequestId per scene (via
-- crypto.randomUUID()) so each job is distinct.
--
-- The bug: the duplicate guard in generator_start_job_v2 was:
--   AND (_client_request_id IS NULL OR client_request_id IS DISTINCT FROM _client_request_id)
-- This meant: if the NEW request has a clientRequestId, then ANY existing job
-- with a DIFFERENT (or NULL) clientRequestId is considered a duplicate.
--
-- The fix: skip the prompt-based duplicate guard entirely when clientRequestId
-- is provided. The pg_advisory_xact_lock + early-return (lines 35-44) already
-- handles idempotency for same-clientRequestId retries. Prompt-based dedup
-- should only apply when NO clientRequestId is provided (legacy/manual submits).
...
BEGIN
  ...
  -- Prompt-based duplicate guard: only applies when NO clientRequestId is
  -- provided. When clientRequestId is present, the lock + early-return above
  -- already ensures idempotency, and unique clientRequestIds per scene prevent
  -- false duplicates in multi-scene (30s/45s/135s) flows.
  ...
  IF _quota.used_this_month + _cost > _quota.monthly_limit_credits THEN
    RAISE EXCEPTION 'quota_exceeded_monthly: % of % credits used today (need % more)',
