...
const DEFAULT_JSON_BODY_MAX_BYTES = 16_384;
...
export async function readJsonBody<T>(
  ...
  try {
    ...
    return {
      ok: false,
      response: errorResponse(req, "INVALID_JSON", "Invalid JSON body", 400, requestId),
    };
  }
  ...
  if (!raw.trim()) {
    return {
      ok: false,
      response: errorResponse(req, "INVALID_JSON", "JSON body required", 400, requestId),
    };
  }
