...
export function buildPreviewShotQualityPrompt(input: PreviewShotQualityInput): string {
  ...
  const product = input.productName?.trim() || "the selected product shown in the image";
  ...
  return [
    ...
    "2. productRelevance: the visible action clearly concerns and promotes the selected product; reject unrelated objects, actions, or a substitute product.",
